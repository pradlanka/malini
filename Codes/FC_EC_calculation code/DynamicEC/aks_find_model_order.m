%-----------------------------------------------------------------------
% FUNCTION: aks_find_model_order.m
...
function [bic,aic,bc,ac] = aks_find_model_order(X,MINP,MAXP)
...
clear bc ac;
for i = MINP:MAXP
    eval('res = aks_regress(X,i);','res = -1');   % estimate regression model, catch errors
    if(~isnumeric(res))
