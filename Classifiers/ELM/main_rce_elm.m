...
hypeelm2=handles.hypeelm2;
lowv=str2double(hypeelm{1,1});
incred=str2double(hypeelm{2,1});
uppe=str2double(hypeelm{3,1});
lowto=str2double(hypeelm2{1,1});
incredto=str2double(hypeelm2{2,1});
uppeto=str2double(hypeelm2{3,1});
exppara=strcmp('yes',hypeelm{4,1});
exparra2=strcmp('yes',hypeelm2{4,1});;
resa=handles.resa;
nocl=handles.nocl;
nofo=handles.nofo;
...
select_significant_paths = SSP;% if you want the algoithm to perform an initial filtering step(Only significantly different paths are selected either by t-test/ANOVA on the train data)
 % if you want the algoithm to perform an initial filtering step(Only significantly different paths are selected either by t-test/ANOVA on the train data)
...
if(arerce==1)
CurrentFolder=pwd;
addpath(genpath(fullfile(CurrentFolder,'Classifiers','ELM','Classifier_Files')));
...
if(exparra2==1)
hyperparameter(1,1)=lowto;
dummy=0;i=1;
while(dummy<=uppeto)
    dummy = lowto*incredto;
i=i+1;
lowto=dummy;
    if(dummy>uppeto)
        break;
...
Hyperparameter{2}=hyperparameter;
else
%Hyperparameter{1} =(lowv : incred : uppe);
Hyperparameter{2} =(lowto : incredto : uppeto);
end
%Hyperparameter{1} = [0.01 0.03 0.1 0.3 1 3 10 30 100];
%Hyperparameter{2} = [0.01 0.03 0.1 0.3 1 3 10 30 100];
...
cd(CurrentFolder);
rmpath(genpath(fullfile(CurrentFolder,'Classifiers','ELM','Classifier_Files')));
