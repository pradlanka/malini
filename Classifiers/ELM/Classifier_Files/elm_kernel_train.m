function [Decision_Surf] = elm_kernel_train(TrainingData_File, Elm_Type, Regularization_coefficient, Kernel_type, Kernel_para)
...
%%%%%%%%%% Calculate training & testing classification accuracy
    ...
    for i = 1 : size(T, 2)
        [x, label_index_predicted]=max(T(:,i));
        [x, label_index_actual]=max(Y(:,i));
        if label_index_actual~=label_index_predicted
...
Decision_Surf = struct('Elm_Type',Elm_Type, 'Regularization_coefficient', Regularization_coefficient,'Kernel_type',Kernel_type, 'Kernel_para', Kernel_para,'label', label, 'NumberofOutputNeurons', NumberofOutputNeurons, 'OutputWeight',OutputWeight,'P',P);
