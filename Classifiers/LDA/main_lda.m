...
if(arerce==1)
CurrentFolder=pwd;
addpath(genpath(fullfile(CurrentFolder,'Classifiers','LDA','Classifier_Files')));
...
rmpath(fullfile(CurrentFolder,'Classifiers','RCE_Classifiers','RCE-LinearSVM','RCE','classify'));
if any(strcmp('Parallel Computing Toolbox', {v.Name}));
   delete(gcp('nocreate'));
end
datestr(now)
...
fclose(fileID);
fprintf('Total time = %5.3f hours\n',time/3600);
rmpath(genpath(fullfile(Currentfolder,'Classifiers','LDA','Classifier_Files')));
