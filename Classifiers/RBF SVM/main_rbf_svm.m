...
hyperbf=handles.hyperbf;
hyperbf2=handles.hyperbf2;
lowv=str2double(hyperbf{1,1});
incred=str2double(hyperbf{2,1});
uppe=str2double(hyperbf{3,1});
lowto=str2double(hyperbf2{1,1});
incredto=str2double(hyperbf2{2,1});
uppeto=str2double(hyperbf2{3,1});
exppara=strcmp('yes',hyperbf{4,1});
exparra2=strcmp('yes',hyperbf2{4,1});;
resa=handles.resa;
nocl=handles.nocl;
nofo=handles.nofo;
...
if(arerce==1)
CurrentFolder=pwd;
addpath(genpath(fullfile(CurrentFolder,'Classifiers','RBF SVM','Classifier_Files')));
...
cd(CurrentFolder);
rmpath(genpath(fullfile(CurrentFolder,'Classifiers','RBF SVM','Classifier_Files')));
