...
hypeknn=handles.hypeknn;
lowv=str2double(hypeknn{1,1});
incred=str2double(hypeknn{2,1});
uppe=str2double(hypeknn{3,1});
exppara=strcmp('yes',hypeknn{4,1});
resa=handles.resa;
nocl=handles.nocl;
nofo=handles.nofo;
...
select_significant_paths = SSP; identify_important_paths = false;% if you want the algoithm to perform an initial filtering step(Only significantly different paths are selected either by t-test/ANOVA on the train data)
...
CurrentFolder=pwd;
addpath(genpath(fullfile(CurrentFolder,'Classifiers','KNN','Classifier_Files')));
...
cd(CurrentFolder);
rmpath(genpath(fullfile(CurrentFolder,'Classifiers','KNN','Classifier_Files')));
...
if identify_important_paths
    mkdir(fullfile(Currentfolder,'Outside_Rce_Results',discos,'Feature_Significance'))
end
