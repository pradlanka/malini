...
nocl=handles.nocl;
nofo=handles.nofo;
hypervm=handles.hypervm;
lowv=str2double(hypervm{1,1});
incred=str2double(hypervm{2,1});
uppe=str2double(hypervm{3,1});
exppara=strcmp('yes',hypervm{4,1});
...
CurrentFolder = pwd;
addpath(genpath(fullfile(CurrentFolder,'Classifiers','RVM','Classifier_Files')));
...
%Hyperparameter{1} = [0.0001 0.0003 0.001 0.003 0.01 0.03 0.1 0.3 1 3 10 30 100];
...
rmpath(genpath(fullfile(CurrentFolder,'Classifiers','RVM','Classifier_Files')));
